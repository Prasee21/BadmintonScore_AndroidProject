...
public class PlayerList extends AppCompatActivity{
    ...
    private void displayPlayerHistory() {
        try {
            SharedPreferences sharedPreferences = getSharedPreferences("PlayerHistory", MODE_PRIVATE);
            String matchResult = sharedPreferences.getString("playerName", "");

            if (!matchResult.isEmpty()) {
                playerListDescription.setText(matchResult);
            } else {
                playerListDescription.setText("No player history available.");
            }
        } catch (Exception e) {
            e.printStackTrace();
